...
string __version = "[checker 5.0] by ycp";
...
void usage() {
    puts("usage: ");
    puts("\n./checker ($problem_name) (-l) (-c) (-q) (-v) (-f)\n");
    ...
    puts("-f: fast mode");
    puts("\n---------\nexample: ./checker prob1 -lfc \n(run checker in fast mod, load problem file \"prob1\" and always continue.)");
    printf("\n---------\n%s\n", __version.c_str());
...
void load_cmd(string cmd)  {
    for (int i = 1; i < cmd.length(); i++) {
        switch (cmd[i]) {
            ...
            case 'q': always_continue = 0, always_quit = 0; break;
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i <= T; i++) {
        ...
        if (errorflag == 1) printf(NONE"time1: %lldms (return 0x%x)", b_time - a_time, WEXITSTATUS(ret));
        ...
        if (errorflag == 2) printf(NONE"time2: %lldms (return 0x%x)\n", b_time - a_time, WEXITSTATUS(ret));
        else printf(NONE"\n" NONE"time2: %lldms\n", b_time - a_time);
        if (errorflag) {
            err342:
